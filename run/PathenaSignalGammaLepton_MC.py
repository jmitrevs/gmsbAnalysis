...
for i,inDS in enumerate(inDS_full):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS=%s/ --outDS=user.jmitrevs.GM_141103.%s --nGBPerJob=MAX newSignalGammaMuon.py" % (inDS, outName)

   
    if i == 0:
        command += " --outTarBall=/data/jmitrevs/scratch/submission_141103.tar"
    else:
        command += " --inTarBall=/data/jmitrevs/scratch/submission_141103.tar"
