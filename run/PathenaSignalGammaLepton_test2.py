...
tarfile = "/data/jmitrevs/scratch/submission_150116.tar"

for i,inDS in enumerate(inDS_test):

    outName = inDS[:88] # make sure the name is not too long
 
    # command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GE_141119.%s newSignalGammaElectron.py " % (inDS, outName)
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GE_150116.%s newSignalGammaElectron_afast.py " % (inDS, outName)
...
for i,inDS in enumerate(inDS_test_mu):
    ...
    #command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GM_141127.%s newSignalGammaMuon.py " % (inDS, outName)
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GM_150116.%s newSignalGammaMuon_afast.py " % (inDS, outName)
