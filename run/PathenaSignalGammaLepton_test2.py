...
for i,inDS in enumerate(inDS_test):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GE_150116.%s newSignalGammaElectron.py " % (inDS, outName)
...
for i,inDS in enumerate(inDS_test_mu):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GM_150116.%s newSignalGammaMuon.py " % (inDS, outName)
