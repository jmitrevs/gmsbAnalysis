...
for i,inDS in enumerate(inDS_full):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GM_120625.%s SignalGammaMuon.py " % (inDS, outName)
    
    # if i == 0:
    #     command += " --outTarBall=/data3/jmitrevs/submission_gammamu_120625.tar"
    # else:
...
for i,inDS in enumerate(inDS_Wjets):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GML_120625.%s SignalGammaMuon_WjetsLoose.py " % (inDS, inDS)
