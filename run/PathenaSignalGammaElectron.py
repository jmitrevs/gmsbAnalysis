...
for i,inDS in enumerate(inDS_full):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --excludedSite=ANALY_GRIF-LAL --inDS %s/ --outDS user.jmitrevs.GE_120519.%s SignalGammaElectron.py " % (inDS, outName)
...
for i,inDS in enumerate(inDS_Zjets):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GE_120519.%s SignalGammaElectron_Zjets.py " % (inDS, inDS)
...
for i,inDS in enumerate(inDS_Ztau):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GE_120519.%s SignalGammaElectron_Ztaujets.py " % (inDS, inDS)
