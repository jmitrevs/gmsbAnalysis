...
for i,inDS in enumerate(inDS_full):
    ...
    command = "pathena --mergeOutput --nGBPerJob=MAX --excludedSite=ANALY_MANC --inDS %s/ --outDS user.jmitrevs.GE_120625.%s SignalGammaElectron.py " % (inDS, outName)
   
    # if i == 0:
    #     command += " --outTarBall=/data3/jmitrevs/submission_gammael_120625.tar"
    # else:
...
for i,inDS in enumerate(inDS_Wjets):
    ...
    # command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.37.%s SignalGammaElectron.py " % (inDS, inDS)
    command = "pathena --mergeOutput --nGBPerJob=MAX --inDS %s/ --outDS user.jmitrevs.GEL_120625.%s SignalGammaElectron_WjetsLoose.py " % (inDS, inDS)
